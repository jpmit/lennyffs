...
ts, nx, bop = readwrite.r3col('times.out')
...
for i in range(nint):
    fin = open('interface%d.out' %(i+1), 'r')
...
# write all data out to file (this file is for debug, see 'allinterfaceseff.out')
sname = 'allinterfaces.out'
# probability of reaching the FINAL interface, starting from lambda1
prob = np.product(probs)
# rate of forming FINAL PHASE, starting from phase A
rate = flux*prob
fout = open(sname,'w')
fout.write('SUMMARY\nCritint Critn\n%d %d\nFlux Prob Rate ln(Rate)\n%.6e %.6e %.6e %.6e\n'
           %(critint,critop,flux,prob,rate,np.log(rate)))
fout.write('----------\nDETAILED BREAKDOWN\n' \
           'phase A OP: %d lambda0 OP: %d phase B OP: %d\n' \
           '%d shots fired from A to lambda0\n' \
           'interface OP fired success P(success) P(finalphase)\n'
           %(params['lambdaA'],params['lambdas'][0],params['lambdas'][-1],len(ts)))
fstr = ''
for i in range(nint):
    fstr = '%s%d->%d %d %d %d %.6f %.6e\n'\
           %(fstr,i,i+1,params['lambdas'][i+1],fired[i],success[i],probs[i],cumprobs[i])
fout.write(fstr)
fout.close()
...
fout = open(sname,'w')
fout.write('SUMMARY\nCritint Critn\n%d %d\nFlux Prob Rate ln(Rate)\n%.6e %.6e %.6e %.6e\n'
           %(critinteff,critopeff,flux,probeff,rateeff,np.log(rateeff)))
fout.write('----------\nDETAILED BREAKDOWN\n' \
           'phase A OP: %d lambda0 OP: %d phase B OP: %d\n' \
           '%d shots fired from A to lambda0\ninterface '\
...
for i in range(nint):
    fstr = '%s%d->%d %d %d %d %.6f %.6e\n'\
           %(fstr,i,i+1,params['lambdas'][i+1],firedeff[i],successeff[i],probseff[i],cumprobseff[i])
fout.write(fstr)
fout.close()
